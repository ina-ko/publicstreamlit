...
def derived_features_chart(project_id: str, model_id: str, selected_class: Optional[str]):
    ...
    if project.target_type == TARGET_TYPE.MULTICLASS:
        ...
        c = alt.Chart(feature_impact).mark_bar().encode(y=alt.Y('featureName', sort='-x'), x=alt.X('impactNormalized'))
